...
export function ExpenseEntry({ settings, accounts, profiles = [], center, onSaved, onClose, linkedShipment, onClearShipment }) {
    ...
    const save = async event => {
        ...
        try {
            if (!id) {
                const result = await apiClient.recordAccountingEntry({ ...entry, kind: 'expense', shipment_id: linkedShipment?.id || null, center: center || null, category: entry.category.trim(), vendor: entry.vendor.trim(), account: entry.account.trim(), reference: entry.reference.trim(), amount: Number(entry.amount) });
            ...
            setMessage('Expense saved successfully.'); onClearShipment?.(); onSaved();
        } catch (error) {
    ...
    return <aside className="ao-panel ao-entry">
        <header><h3>Expense Entry</h3><span className="ao-quick">Quick Add</span><button aria-label="Close expense entry" disabled={saving} onClick={onClose}><X size={14} /></button></header>
        <form onSubmit={save}>
            <fieldset disabled={saving || !!savedId}>
                ...
                <datalist id="ao-payment-accounts">{accounts.map(name => <option key={name} value={name} />)}</datalist>
            ...
            <label>Upload Bill (Optional)<input ref={fileInput} type="file" accept=".pdf,.png,.jpg,.jpeg" onChange={e => setFile(e.target.files?.[0] || null)} /><small>PDF, PNG or JPEG · Up to 5 MB</small></label>
            ...
            <button className="ao-button primary" disabled={saving}>{saving ? 'Saving…' : savedId ? 'Retry Bill Upload' : 'Save Expense'}</button>
