...
export function ShipmentLedger({ report, accounts, filters, setFilters, onSearch, ledger, page, setPage, refresh, onViewShipment, onAddShipmentExpense }) {
    ...
    return <section className="ao-panel ao-ledger">
        ...
        <div className="ao-table-scroll" tabIndex={0} role="region" aria-label="Shipment accounts ledger">
            <table className="ao-table"><thead><tr>{['#', 'Date', 'AWB No.', 'Courier', 'Customer', 'Destination', 'Sale (₹)', 'Courier Cost (₹)', 'Expense (₹)', 'Value (₹)', 'Payment Mode', 'Collection Status', 'Payment to Courier', 'Action'].map(name => <th key={name}>{name}</th>)}</tr></thead>
                <tbody>{!ledger.loading && items.map((s, index) => <tr key={s.id}>
                    <td>{(page - 1) * 5 + index + 1}</td><td>{dateLabel(s.date)}</td><td><TrackingLink awb={s.awb} courier={s.courier} /></td><td><CourierLogo courier={s.courier} height={13} /></td>
                    <td title={s.customer_name}>{s.customer_name}</td><td>{s.destination}</td><td>{money(s.sale)}</td><td>{money(s.cost)}</td><td>{money(s.expense)}</td><td className={s.value < 0 ? 'negative' : 'positive'}><strong>{money(s.value)}</strong></td>
                    <td>{s.payment_mode || '—'}</td><td><span className={`ao-status ${s.collection_status === 'Paid' ? 'paid' : 'due'}`}>{s.collection_status === 'Paid' ? 'Collected' : s.collection_status}</span></td>
                    <td><span className={`ao-status ${s.courier_status === 'Wallet debited' ? 'paid' : 'due'}`} title="Provider payouts are not allocated to individual shipments">{s.courier_status || '—'}</span></td>
                    <td><div className="ao-row-actions"><button className="ao-eye" aria-label={`View shipment ${s.awb}`} onClick={() => onViewShipment(s)}><Eye size={13} /></button>{onAddShipmentExpense && <button className="ao-eye" aria-label={`Add expense to shipment ${s.awb}`} title="Add shipment expense" onClick={() => onAddShipmentExpense(s)}><MoreHorizontal size={13} /></button>}</div></td>
                </tr>)}
                {(ledger.loading || !items.length) && <tr><td colSpan={14} className="ao-empty ao-ledger-empty">{ledger.loading ? 'Loading shipments…' : 'No shipments match this period and filters.'}</td></tr>}
                {!ledger.loading && items.length > 0 && items.length < 5 && Array.from({ length: 5 - items.length }, (_, i) => <tr aria-hidden="true" className="ao-blank-row" key={`blank-${i}`}><td colSpan={14}>&nbsp;</td></tr>)}
                </tbody>
            </table>
        </div>
...
export function AccountsCharts({ report, data, range, go, refresh }) {
    ...
    return <>
        <div className="ao-charts">
            <Breakdown title="Expense Breakdown" data={report?.by_category} label="Total Expenses" onDetails={() => go('expenses')} />
            <Breakdown title="Collection by Payment Mode" data={report?.by_mode} label="Collected" onDetails={() => go('collections')} />
            <section className="ao-panel"><header><h3>Courier Cost by Partner</h3><button onClick={() => go('postpaid')}>View details →</button></header>
