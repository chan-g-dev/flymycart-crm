    ...
    return <section id="account-checks" className="account-nav-target account-checks">
        <div className="dash-box">
            ...
            <div className="table-wrap"><table className="data-table"><thead><tr><th>Date</th><th>AWB</th><th>Customer</th><th>Amount</th><th>Method</th><th>Paid to</th><th>Collected by</th><th>Reference</th></tr></thead><tbody>
                {receiptResult.error ? <tr><td colSpan="8" role="alert">{receiptResult.error} <button className="btn btn-sm btn-outline" onClick={() => setRevision(value => value + 1)}>Retry</button></td></tr> : !receipts ? <tr><td colSpan="8"><LoadingSpinner inline text="Loading receipts..." /></td></tr> : receipts.items.length === 0 ? <tr><td colSpan="8">No receipts in this period.</td></tr> : receipts.items.map(r => <tr key={r.id}><td>{r.date}</td><td>{r.awb || '—'}</td><td>{r.customer || '—'}</td><td>{currency(r.amount)}</td><td>{r.payment_method}</td><td>{r.paid_to}</td><td>{r.collected_by}</td><td>{r.reference || '—'}<PaymentDetailsSummary details={r.payment_details} /></td></tr>)}
            </tbody></table></div>
        ...
        <div className="dash-box">
            ...
            {hasPermission('accounts.edit') && <form onSubmit={submit}>
                ...
                </div><button className="btn btn-primary-blue" type="submit">{saving ? <ButtonSpinner text="Recording…" /> : 'Record account check'}</button></fieldset>
            </form>}
            ...
            <div className="table-wrap"><table className="data-table"><thead><tr><th>Receipt date</th><th>Account / center</th><th>Receipts</th><th>Recorded</th><th>Counted</th><th>Result</th><th>Checked by</th><th>Notes</th></tr></thead><tbody>
                {historyResult.error ? <tr><td colSpan="8" role="alert">{historyResult.error} <button className="btn btn-sm btn-outline" onClick={() => setRevision(value => value + 1)}>Retry</button></td></tr> : !history ? <tr><td colSpan="8"><LoadingSpinner inline text="Loading checks..." /></td></tr> : history.items.length === 0 ? <tr><td colSpan="8">No account checks recorded.</td></tr> : history.items.map(c => <tr key={c.id}><td>{c.date}</td><td>{c.account}<br /><small>{c.center || 'All centers'}</small></td><td>{c.receipt_count}</td><td>{currency(c.expected_amount)}</td><td>{currency(c.counted_amount)}</td><td>{c.difference === 0 ? 'Matched' : `${c.difference < 0 ? 'Shortage' : 'Overage'} ${currency(Math.abs(c.difference))}`}</td><td>{c.checked_by}<br /><small>{c.created_at ? formatRecordTime(c.created_at) : ''}</small></td><td>{c.notes || '—'}</td></tr>)}
            </tbody></table></div>
